"""Test Flujo 2 Recaudaciones - 11 types Newcore -> Legacy con anti-loop check."""
...
# (agg, oracle dest, safe col noop, legacy table dbRC)
TESTS=[
    ('aplicacionRecaudacion_type','APLICACIONRECAUDACION_TYPE','MONTORECA','rctbapli_reca'),
    ('caucionRecaudacion_type','CAUCIONRECAUDACION_TYPE','TIPOREVE','rctbcaut'),
    ('devolucionRendicion_type','DEVOLUCIONRENDICION_TYPE','MONTODEVO','rctbdevo_rind'),
    ('estadoCuenta_type','ESTADOCUENTA_TYPE','MONTODEPO','rctbesta_cnta'),
    ('estadoRecaudacion_type','ESTADORECAUDACION_TYPE','DESCRIPCIONESTADO','rctbesta_reca'),
    ('recaudacion_type','RECAUDACION_TYPE','MONTO_RECA','rctbreca'),
    ('rendicion_type','RENDICION_TYPE','NUMERO_CPBT','rctbrind'),
    ('saldoRecaudacion_type','SALDORECAUDACION_TYPE','MONTOGENE','rctbcsal_reca'),
    ('sciRecaudacion_type','SCIRECAUDACION_TYPE','NUMEROENVI','rctbcsci_bce'),
    ('tipoDescuento_type','TIPODESCUENTO_TYPE','NOMBRETIPODESC','rctbtipo_desc'),
    ('transaccionRecaudacion_type','TRANSACCIONRECAUDACION_TYPE','NOMBRERECA','rctbtrec'),
]
...
for agg,_,_,ltbl in TESTS:
    rc.execute(f'SELECT COUNT(*) FROM dbo.[{ltbl}]')
    legacy_before[ltbl]=rc.fetchone()[0]
...
for i,(agg,dest,col,ltbl) in enumerate(TESTS):
    try:
        ...
        r=o.fetchone()
        if not r:
            print(f'  [{i+1:>2}] {agg:<32} {dest:<32} tabla vacia, skip', flush=True)
            continue
        rid=r[0]
        o.execute(f"UPDATE FCME_USER.{dest} SET {col}={col} WHERE ID=:1", [rid])
        orcl.commit()
        fired+=1
        print(f'  [{i+1:>2}] {agg:<32} {dest:<32} (ID={rid}) UPDATE noop', flush=True)
    except Exception as e:
        print(f'  [{i+1:>2}] {agg:<32} ERR {str(e)[:120]}', flush=True)
...
print(f"{'#':>3} {'aggregate_type':<32} {'inbox':<6} {'proc':<5} {'err':<5} {'leg_delta':<10} status")
print('-'*120)
ok=0
for i,(agg,dest,col,ltbl) in enumerate(TESTS):
    ...
    if inb_n>=1 and pr_n>=inb_n and er_n==0:
        status='OK'; ok+=1
    elif er_n>0: status=f'ERR: {em[:50]}'
    else: status='NO INBOX'
    print(f'{i+1:>3} {agg:<32} {inb_n:<6} {pr_n:<5} {er_n:<5} {legacy_delta:+}{"":<7} {status[:60]}', flush=True)
...
can.execute(f"SELECT COUNT(*) FROM dbo.cdc_outbox WHERE source_table LIKE 'dbRC%' AND created_at > DATEADD(MINUTE,-2,SYSDATETIME())")
flow1_echo=can.fetchone()[0]
print(f'  Flujo 1 echo dbRC ultimos 2 min: {flow1_echo}', flush=True)

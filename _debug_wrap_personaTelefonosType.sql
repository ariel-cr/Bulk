CREATE OR ALTER PROCEDURE dbo.usp_inbox_personaTelefonosType
...
BEGIN
    SET NOCOUNT ON;
    BEGIN TRY
        DECLARE @pk_str NVARCHAR(200) = JSON_VALUE(@payload,'$.EMPRESAOPERADORA');
        IF @pk_str IS NULL
        BEGIN
            INSERT INTO dbo.cdc_inbox_errors (inbox_id, aggregate_type, event_type, error_message)
            VALUES (@inbox_id, @aggregate_type, @event_type, N'payload sin EMPRESAOPERADORA');
            RETURN;
        END
        DECLARE @accion CHAR(1) = CASE
            ...
            ELSE 'I' END;
        DECLARE @pk SMALLINT = TRY_CAST(JSON_VALUE(@payload,'$.EMPRESAOPERADORA') AS SMALLINT);
        IF @pk IS NULL
        BEGIN
            INSERT INTO dbo.cdc_inbox_errors (inbox_id, aggregate_type, event_type, error_message)
        ...
        EXEC dbFC.dbo.sp_PERSONATELEFONOSTYPE_CRUD
            @Accion = @accion,
            @PK = @pk,
        @ci_tipo = JSON_VALUE(@payload,'$.CODIGOTIPOIDENTIFICACION'),
        @ci_cedula = JSON_VALUE(@payload,'$.IDENTIFICACION'),
        @tx_telefono = JSON_VALUE(@payload,'$.SECUENCIATELEFONO'),
        @no_direccion = JSON_VALUE(@payload,'$.NUMERODIRECCION'),
        @fx_creacion = TRY_CAST(JSON_VALUE(@payload,'$.FECHAINGRESO') AS DATETIME);
    END TRY
    BEGIN CATCH
        INSERT INTO dbo.cdc_inbox_errors (inbox_id, aggregate_type, event_type, error_message)
        VALUES (@inbox_id, @aggregate_type, @event_type, N'wrapper personaTelefonosType: ' + ERROR_MESSAGE());
    END CATCH
END
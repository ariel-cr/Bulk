CREATE OR ALTER PROCEDURE dbo.usp_inbox_personaReferenciasPersonalesType
...
BEGIN
    ...
    BEGIN TRY
        ...
        EXEC dbFC.dbo.sp_PERSONAREFERENCIASPERSONALESTYPE_CRUD
            @Accion = @accion,
            @PK = @pk,
        @no_nomb = JSON_VALUE(@payload,'$.NOMBRESPERSONA'),
        @no_apel = JSON_VALUE(@payload,'$.APELLIDOPATERNO');
    END TRY
    BEGIN CATCH
        INSERT INTO dbo.cdc_inbox_errors (inbox_id, aggregate_type, event_type, error_message)
        VALUES (@inbox_id, @aggregate_type, @event_type, N'wrapper personaReferenciasPersonalesType: ' + ERROR_MESSAGE());
    END CATCH
END
...
BEGIN
    SET NOCOUNT ON;
    BEGIN TRY
        DECLARE @pk_str NVARCHAR(200) = JSON_VALUE(@payload,'$.CODIGOTIPOREFERENCIA');
        IF @pk_str IS NULL
        BEGIN
        ...
        DECLARE @accion CHAR(1) = CASE
            ...
            ELSE 'I' END;
        DECLARE @pk SMALLINT = TRY_CAST(JSON_VALUE(@payload,'$.CODIGOTIPOREFERENCIA') AS SMALLINT);
        IF @pk IS NULL
        BEGIN
            INSERT INTO dbo.cdc_inbox_errors (inbox_id, aggregate_type, event_type, error_message)
        ...
        VALUES (@inbox_id, @aggregate_type, @aggregate_id, @event_type, @pk_str,
                JSON_VALUE(@payload,'$.CODIGOTIPOREFERENCIA'));

        EXEC dbFC.dbo.sp_REFERENCIAPARTICIPE_TYPE_CRUD
            @Accion = @accion,
            @PK = @pk,
        @ds_tref = JSON_VALUE(@payload,'$.DESCRIPCIONTIPOREFERENCIA');
    END TRY
    BEGIN CATCH
        INSERT INTO dbo.cdc_inbox_errors (inbox_id, aggregate_type, event_type, error_message)
        VALUES (@inbox_id, @aggregate_type, @event_type, N'wrapper referenciaParticipeType: ' + ERROR_MESSAGE());
    END CATCH
END
CREATE OR ALTER PROCEDURE dbo.usp_inbox_reporteSIBSParticipeType
...
BEGIN
    SET NOCOUNT ON;
    BEGIN TRY
        DECLARE @pk_str NVARCHAR(200) = JSON_VALUE(@payload,'$.NUMEROREGISTRO');
        IF @pk_str IS NULL
        BEGIN
            INSERT INTO dbo.cdc_inbox_errors (inbox_id, aggregate_type, event_type, error_message)
            VALUES (@inbox_id, @aggregate_type, @event_type, N'payload sin NUMEROREGISTRO');
            RETURN;
        END
        DECLARE @accion CHAR(1) = CASE
            ...
            ELSE 'I' END;
        DECLARE @pk INT = TRY_CAST(JSON_VALUE(@payload,'$.NUMEROREGISTRO') AS INT);
        IF @pk IS NULL
        BEGIN
            INSERT INTO dbo.cdc_inbox_errors (inbox_id, aggregate_type, event_type, error_message)
        ...
        VALUES (@inbox_id, @aggregate_type, @aggregate_id, @event_type, @pk_str,
                JSON_VALUE(@payload,'$.CODIGOENTIDAD'));

        EXEC dbFC.dbo.sp_REPORTESIBSPARTICIPE_TYPE_CRUD
            @Accion = @accion,
            @PK = @pk,
        @ci_cedu = JSON_VALUE(@payload,'$.NUMEROCEDULA'),
        @ti_iden = JSON_VALUE(@payload,'$.TIPOIDENTIFICACION');
    END TRY
    BEGIN CATCH
        INSERT INTO dbo.cdc_inbox_errors (inbox_id, aggregate_type, event_type, error_message)
        VALUES (@inbox_id, @aggregate_type, @event_type, N'wrapper reporteSIBSParticipeType: ' + ERROR_MESSAGE());
    END CATCH
END
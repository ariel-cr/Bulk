CREATE OR ALTER PROCEDURE dbo.usp_inbox_saldoDiarioRubroType
...
BEGIN
    SET NOCOUNT ON;
    BEGIN TRY
        DECLARE @pk_str NVARCHAR(200) = JSON_VALUE(@payload,'$.CODIGOMOTIVO');
        IF @pk_str IS NULL
        BEGIN
            INSERT INTO dbo.cdc_inbox_errors (inbox_id, aggregate_type, event_type, error_message)
            VALUES (@inbox_id, @aggregate_type, @event_type, N'payload sin CODIGOMOTIVO');
            RETURN;
        END
        DECLARE @accion CHAR(1) = CASE
            ...
            ELSE 'I' END;
        DECLARE @pk CHAR(3) = CAST(JSON_VALUE(@payload,'$.CODIGOMOTIVO') AS CHAR(3));
        IF @pk IS NULL
        BEGIN
            INSERT INTO dbo.cdc_inbox_errors (inbox_id, aggregate_type, event_type, error_message)
        ...
        VALUES (@inbox_id, @aggregate_type, @aggregate_id, @event_type, @pk_str,
                JSON_VALUE(@payload,'$.FECHASALDO'));

        EXEC dbFC.dbo.sp_SALDODIARIORUBRO_TYPE_CRUD
            @Accion = @accion,
            @PK = @pk,
        @fx_saldo = TRY_CAST(JSON_VALUE(@payload,'$.FECHASALDO') AS DATETIME),
        @co_empr = TRY_CAST(JSON_VALUE(@payload,'$.CODIGOEMPRESA') AS SMALLINT);
    END TRY
    BEGIN CATCH
        INSERT INTO dbo.cdc_inbox_errors (inbox_id, aggregate_type, event_type, error_message)
        VALUES (@inbox_id, @aggregate_type, @event_type, N'wrapper saldoDiarioRubroType: ' + ERROR_MESSAGE());
    END CATCH
END
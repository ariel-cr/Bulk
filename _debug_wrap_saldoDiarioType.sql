CREATE OR ALTER PROCEDURE dbo.usp_inbox_saldoDiarioType
...
BEGIN
    SET NOCOUNT ON;
    BEGIN TRY
        DECLARE @pk_str NVARCHAR(200) = JSON_VALUE(@payload,'$.FECHASALDO');
        IF @pk_str IS NULL
        BEGIN
            INSERT INTO dbo.cdc_inbox_errors (inbox_id, aggregate_type, event_type, error_message)
            VALUES (@inbox_id, @aggregate_type, @event_type, N'payload sin FECHASALDO');
            RETURN;
        END
        DECLARE @accion CHAR(1) = CASE
            ...
            ELSE 'I' END;
        DECLARE @pk DATETIME = TRY_CAST(JSON_VALUE(@payload,'$.FECHASALDO') AS DATETIME);
        IF @pk IS NULL
        BEGIN
            INSERT INTO dbo.cdc_inbox_errors (inbox_id, aggregate_type, event_type, error_message)
        ...
        EXEC dbFC.dbo.sp_SALDODIARIO_TYPE_CRUD
            @Accion = @accion,
            @PK = @pk,
    ...
    BEGIN CATCH
        INSERT INTO dbo.cdc_inbox_errors (inbox_id, aggregate_type, event_type, error_message)
        VALUES (@inbox_id, @aggregate_type, @event_type, N'wrapper saldoDiarioType: ' + ERROR_MESSAGE());
    END CATCH
END
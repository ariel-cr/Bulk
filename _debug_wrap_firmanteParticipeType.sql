CREATE OR ALTER PROCEDURE dbo.usp_inbox_firmanteParticipeType
...
BEGIN
    ...
    BEGIN TRY
        ...
        EXEC dbFC.dbo.sp_FIRMANTEPARTICIPE_TYPE_CRUD
            @Accion = @accion,
            @PK = @pk,
        @ci_institucion = JSON_VALUE(@payload,'$.CODIGOINSTITUCION'),
        @sc_firmante = TRY_CAST(JSON_VALUE(@payload,'$.NOFIRMANTE') AS SMALLINT),
        @ci_cedula = JSON_VALUE(@payload,'$.NUMEROCEDULA'),
        @ci_tipo = JSON_VALUE(@payload,'$.TIPOINSTITUCION');
    END TRY
    BEGIN CATCH
        INSERT INTO dbo.cdc_inbox_errors (inbox_id, aggregate_type, event_type, error_message)
        VALUES (@inbox_id, @aggregate_type, @event_type, N'wrapper firmanteParticipeType: ' + ERROR_MESSAGE());
    END CATCH
END